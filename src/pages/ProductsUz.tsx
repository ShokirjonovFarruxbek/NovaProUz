...
const ProductsUz = () => {
  ...
  const getTranslatedContent = (content: any) => {
    if (typeof content === 'object' && content !== null) {
      return content.uz || content;
    }
    return content;
  };

  const services = t.products.items.map(item => ({
    id: getTranslatedContent(item.name) === 'HR Xizmatlar' ? 'hr' : 
         getTranslatedContent(item.name) === 'MySklad' ? 'mysklad' : 'website',
